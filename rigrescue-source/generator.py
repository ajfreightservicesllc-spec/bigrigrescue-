...
def render_header(title="Big Rig Rescue", canonical_url=None):
    if canonical_url is None:
        canonical_url = "https://bigrigrescue.co"
...
def render_footer():
    return """    </div>
    <footer>
        <p>&copy; 2024 Big Rig Rescue. 24/7 Emergency Truck Repair & Roadside Service.
...
def generate_homepage(shops, states, cities_dict):
    ...
    html = render_header("Big Rig Rescue - 24/7 Emergency Truck Repair on I-40")
...
def generate_shop_page(shop, state_code, city):
    ...
    canonical_url = f"https://bigrigrescue.co/shop/{state_code}/{slugify(city)}/{shop_slug}/"
    ...
    html = render_header(title, canonical_url)
    html += f"""
...
def generate_state_page(state_code, shops):
    ...
    canonical_url = f"https://bigrigrescue.co/state/{state_code}/"
...
def generate_city_page(state_code, city, shops):
    ...
    canonical_url = f"https://bigrigrescue.co/city/{state_code}/{slugify(city)}/"
...
"""
    ...
    for state_code in sorted(states):
        sitemap += f"""    <url>
        <loc>https://bigrigrescue.co/state/{state_code}/</loc>
        <changefreq>weekly</changefreq>
...
"""
    ...
    for city_state in sorted(cities_dict.keys())[:100]:
        ...
        sitemap += f"""    <url>
        <loc>https://bigrigrescue.co/city/{state_code}/{slugify(city_state.split(", ")[0])}/</loc>
        <changefreq>weekly</changefreq>
...
"""
    ...
    for service_key in ['roadside', '24-7', 'reefer', 'tire', 'cooling', 'engine', 'trailer', 'rv']:
        sitemap += f"""    <url>
        <loc>https://bigrigrescue.co/service/{service_key}/</loc>
        <changefreq>weekly</changefreq>
        <priority>0.8</priority>
    </url>
"""
...
def generate_robots_txt():
    robots = """User-agent: *
Allow: /
Sitemap: https://bigrigrescue.co/sitemap.xml
"""
